...
Rectangle ParseRectangle(const std::string& data)
{
    ...
    auto bottomLeft = Point{ToInt(coordinates[0]), ToInt(coordinates[1])};
    auto topRight = Point{ToInt(coordinates[2]), ToInt(coordinates[3])};
    VerifyRelativePointLocations(bottomLeft, topRight);
    return {bottomLeft, topRight};
}

std::vector<Rectangle> ParseRectangles(const std::string& data)
{
